...
ls("package:unmarked") # list of objects in the package
vcdExtra::datasets("unmarked")
data(package = "unmarked")


#### pcount.spHDS() ####
?pcount.spHDS() #is for fitting an N-mixture model for a discrete state space with raster covariates, and a detection function which decreases with distance from the observer, assumed to be at the centre. See Kery & Royle (2016) Section 9.8.4 for details
...
## 6. Fit some models
(fm0 <- pcount.spHDS(~ -1 + I(dist^2) ~ 1, umf, K = 20)) # formula = double  right-hand  side  formula  describing  covariates  of  detection  and  abundance, in that order
...
#### Examples ####
data(mallard)
mallardUMF <- unmarkedFramePCount(mallard.y,
